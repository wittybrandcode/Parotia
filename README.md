...
Working extension (Chrome MV3), version `0.2.0`. 208 unit/integration tests across 23 files, all green.

Implemented:

- **Freeze** — soft freeze with a stability monitor (`window.stop()`, animation/transition/video suspension).
- **Inspect & clean** — pick elements, then Delete / Hide / Delete Similar, all undoable through a History Engine with an action log and Redo.
...
- **Capture** — full-page capture (viewport slicing + stitching, fixed-header removal), single-element capture (visual isolation, x2 zoom, crop), and **free-select region capture** (draw a rectangle on the page to capture an arbitrary area), exported as `parotia-<title>-<timestamp>.png` via `chrome.downloads` (requested on first export).
...
4. Freeze the page (click the Parotia logo), then use Pick/Delete/Hide, Delete Similar, Undo/Redo, or capture full page / selected element.
